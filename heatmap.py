...
# from pytorch_grad_cam.activations_and_gradients import ActivationsAndGradients # This class is redefined below, so we can comment this out to avoid confusion
...
class yolo_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_result, renormalize,
        ...
        self.task =task
        self.device = device
        model_names = self.model_yolo.names
        print(f'model class info:{model_names}')
        model = copy.deepcopy(self.model_yolo.model)
        model.to(device)
        model.info()
        ...
        if task == 'detect':
            target = yolo_detect_target(backward_type, self.conf_threshold, ratio, model.end2end)
        elif task == 'segment':
            target = yolo_segment_target(backward_type, self.conf_threshold, ratio, model.end2end)
        elif task == 'pose':
            target = yolo_pose_target(backward_type, self.conf_threshold, ratio, model.end2end)
        elif task == 'obb':
            target = yolo_obb_target(backward_type, self.conf_threshold, ratio, model.end2end)
        elif task == 'classify':
            target = yolo_classify_target(backward_type, self.conf_threshold, ratio, model.end2end)
        else:
    ...
    def process(self, img_path, save_path):
        ...
        # ----------------------------------------------------------------
        preprocessor = LetterBox((self.img_size, self.img_size), auto=False, scaleFill=True)
        img_processed_bgr = preprocessor(image=img_bgr)
        # 2. 转换为 Tensor (后续流程保持不变)
        # 注意：现在 img_processed_bgr 已经是目标尺寸 (e.g., 320x320)
        img_rgb = cv2.cvtColor(img_processed_bgr, cv2.COLOR_BGR2RGB)
        img_for_cam = np.float32(img_rgb) / 255.0  # 用于生成 CAM 的图像


        # img, _, (top, bottom, left, right) = letterbox(img_bgr, new_shape=(self.img_size, self.img_size),
        #                                                auto=False)  # 如果需要完全固定成宽高一样就把auto设置为False
        # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # img = np.float32(img) / 255.0
        # tensor = torch.from_numpy(np.transpose(img, axes=[2, 0, 1])).unsqueeze(0).to(self.device)
        # print(f'tensor size:{tensor.size()}')
        tensor = torch.from_numpy(np.transpose(img_for_cam, axes=[2, 0, 1])).unsqueeze(0).to(self.device)
        print(f'tensor size:{tensor.size()}')
        try:
        ...
        cam_image = show_cam_on_image(img_for_cam, grayscale_cam, use_rgb=True, colormap=cv2.COLORMAP_PARULA, image_weight=0.5)
        ...
        if self.renormalize and self.task in ['detect', 'segment', 'pose']:
            cam_image = self.renormalize_cam_in_bounding_boxes(pred.boxes.xyxy.cpu().detach().numpy().astype(np.int32),
                                                               img_for_cam, grayscale_cam)

        # 去掉padding边界
        # cam_image = cam_image[top:cam_image.shape[0] - bottom, left:cam_image.shape[1] - right]

        # ------------------- 核心修改 2: 将图像恢复到原始尺寸 -------------------
        cam_image = cv2.resize(cam_image, (original_w, original_h))
        # --------------------------------------------------------------------
        if self.show_result:
            cam_image = pred.plot(img=cam_image.copy(),
                                  conf=True,  # 显示置信度
...
def get_params():
    params = {
        'weight': 'E:/project_pycharm/midTubedetection/runs/classify/#all_m_v2/weights/best.pt',  # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
        ...
        'task': 'classify',  # 任务(detect,segment,pose,obb,classify)
        'img_size': 320,  # 图像尺寸
    }
    return params
...
if __name__ == '__main__':
    ...
    model(r'dataset_croped/unconfirmed/0', 'result')
